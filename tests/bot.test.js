...
describe('handleMessage', () => {
  ...
  it('does not reply when message is not an expense', async () => {
    shouldProcess.mockReturnValue(true);
    parseExpense.mockResolvedValue({ isExpense: false, expenses: [] });
  ...
  it('appends to sheet and replies with confirmation for single expense', async () => {
    shouldProcess.mockReturnValue(true);
    parseExpense.mockResolvedValue(makeParsed());
  ...
  it('appends multiple rows and replies with item list for multi-expense', async () => {
    shouldProcess.mockReturnValue(true);
    ...
    appendExpense.mockResolvedValue(undefined);
    const msg = makeMessage('coffee 80 snack 100');
    await handleMessage(msg, mockConfig, mockGenAI, mockSheetsClient);
    expect(appendExpense).toHaveBeenCalledTimes(2);
    expect(msg.reply).toHaveBeenCalledWith(expect.stringContaining('2 items'));
    expect(msg.reply).toHaveBeenCalledWith(expect.stringContaining('Coffee'));
    expect(msg.reply).toHaveBeenCalledWith(expect.stringContaining('Snack'));
  });
